...
6. 앱별 공개 `ReportTarget` 카탈로그는 `scripts/generate_targets.py`로 각 앱의 `shipping.yml`에서 생성한다. 앱 ID는 `app.id` 값을 사용하며 디렉터리명이나 저장소명으로 추론하지 않는다. `Sources/ReportKit/ReportTargets.generated.swift`는 수동 편집하지 않는다.
